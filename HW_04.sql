...
SELECT from_user_id 'id_отправителя', 
	(SELECT firstname 
    FROM users 
    WHERE id = messages.from_user_id) as 'Имя', 
	(SELECT lastname 
    FROM users 
    WHERE id = messages.from_user_id) as 'Фамилия', 
    COUNT(*) AS 'Отправлено_сообщений', 
    to_user_id 'id_получателя'
FROM messages 
WHERE to_user_id = 1 AND from_user_id IN (   -- проверяем друзья или нет
...
ORDER BY Отправлено_сообщений DESC;
-- LIMIT 1;